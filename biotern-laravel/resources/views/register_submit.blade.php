...
if ($role === 'student') {
    ...
    if ($password !== $confirm_password) {
        header('Location: /register?registered=error&msg=' . urlencode('Passwords do not match'));
        exit;
    }
    ...
    if ($stmt_user) {
        ...
        try {
            ...
            if ($code === 1062 || strpos($msg, 'Duplicate entry') !== false) {
                header('Location: /register?registered=exists&msg=' . urlencode('An account with that email or username already exists'));
                exit;
            }
            header('Location: /register?registered=error&msg=' . urlencode($msg));
            exit;
        }
        $stmt_user->close();
    }
    ...
    if (!$user_id) {
        header('Location: /register?registered=error&msg=' . urlencode('Failed to create user account'));
        exit;
    }
    ...
    if (!$stmt) {
        header('Location: /register?registered=error&msg=' . urlencode('Database statement error: ' . $mysqli->error));
        exit;
    }
    ...
    try {
            ...
            $error = $stmt->error;
            $stmt->close();
            header('Location: /register?registered=error&msg=' . urlencode('Student record error: ' . $error));
            exit;
        }
        } catch (mysqli_sql_exception $e) {
        $stmt->close();
        header('Location: /register?registered=error&msg=' . urlencode('Student record error: ' . $e->getMessage()));
        exit;
    }

    $stmt->close();

    header('Location: /register?registered=student');
    exit;
}
...
if ($role === 'coordinator') {
    ...
    if ($stmt) {
        $stmt->bind_param('isssiis', $userId, $first_name, $last_name, $final_email, $phone, $department_id, $office_location);
            ...
            $error = $stmt->error;
            $stmt->close();
            header('Location: /register?registered=error&msg=' . urlencode($error));
            exit;
        }
        $stmt->close();
    }

    header('Location: /register?registered=coordinator');
    exit;
}
...
if ($role === 'supervisor') {
    ...
    header('Location: /register?registered=supervisor');
    exit;
}
...
if ($role === 'admin') {
    ...
    header('Location: /register?registered=admin');
    exit;
}
