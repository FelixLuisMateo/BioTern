...
<body>
            ...
            <div class="minimal-card-wrapper">
                <div class="card mb-4 mt-5 mx-2 mx-sm-0 position-relative" style="width: 130%; max-width: 1500px; margin: 40px auto;">
                    ...
                    <div class="card-body p-sm-5" style="padding: 50px !important; min-height: auto;">
                        ...
                        @php
                            $reg = request()->get('registered');
                            $msg = request()->get('msg');
                        @endphp

                        @if($reg === 'exists')
                            <div class="alert alert-warning" role="alert">{{ $msg ?? 'An account with that email or username already exists.' }}</div>
                        @elseif($reg === 'error')
                            <div class="alert alert-danger" role="alert">{{ $msg ?? 'An error occurred while registering.' }}</div>
                        @elseif(in_array($reg, ['student','coordinator','supervisor','admin']))
                            <div class="alert alert-success" role="alert">Registration successful. You may now login.</div>
                        @endif
                        ...
                        <form id="studentForm" class="w-100 mt-4 pt-2 hide-form" action="{{ url('/register_submit') }}" method="post">
                            {{ csrf_field() }}
                        ...
                        <form id="coordinatorForm" class="w-100 mt-4 pt-2 hide-form" action="{{ url('/register_submit') }}" method="post">
                            {{ csrf_field() }}
                        ...
                        <form id="supervisorForm" class="w-100 mt-4 pt-2 hide-form" action="{{ url('/register_submit') }}" method="post">
                            {{ csrf_field() }}
                        ...
                        <form id="adminForm" class="w-100 mt-4 pt-2 hide-form" action="{{ url('/register_submit') }}" method="post">
                            {{ csrf_field() }}
    ...
    <script src="{{ asset('frontend/assets/vendors/js/lslstrength.min.js') }}"></script>
    <!--! END: Vendors JS !-->
    <!--! BEGIN: Apps Init  !-->
