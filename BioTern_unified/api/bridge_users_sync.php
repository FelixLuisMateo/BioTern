...
function bridge_users_sync_ensure_profile_table(mysqli $conn): void
{
...
function bridge_users_sync_ensure_cache_table(mysqli $conn): void
{
...
function bridge_users_sync_request_token(): string
{
...
function bridge_users_sync_token_candidates(array $profile): array
{
    $candidates = [];
...
bridge_users_sync_ensure_profile_table($conn);
bridge_users_sync_ensure_cache_table($conn);
...
$providedToken = bridge_users_sync_request_token();
$authorized = false;
foreach (bridge_users_sync_token_candidates($profile) as $candidate) {
...
if (!is_string($usersJson)) {
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Failed to encode users cache payload']);
    $conn->close();
    exit;
}

$sourceNode = trim((string)($_SERVER['HTTP_X_BRIDGE_NODE'] ?? ''));
$usersCount = count($users);

$stmt = $conn->prepare('INSERT INTO biometric_bridge_user_cache (source_node, users_json, users_count) VALUES (?, ?, ?)');
if (!$stmt) {
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Failed to prepare cache insert']);
    $conn->close();
    exit;
}
