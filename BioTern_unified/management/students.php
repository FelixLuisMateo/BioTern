...
<head>
    ...
    <style>
        ...
        .students-action-dropdown > a.portal-open {
            background: rgba(37, 99, 235, 0.14);
            border-color: rgba(37, 99, 235, 0.45);
            box-shadow: 0 0 0 2px rgba(37, 99, 235, 0.18);
        }

        html.app-skin-dark .students-action-dropdown > a.portal-open {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.students-action-dropdown').forEach(function (dropdownEl) {
                ...
                dropdownEl.addEventListener('hide.bs.dropdown', function () {
                    var toggle = dropdownEl.querySelector('[data-bs-toggle="dropdown"]');
                    var menuEl = document.querySelector('.students-action-menu-portal[data-portal-parent-id="' + dropdownEl.id + '"]')
            ...
            function toCsv(rows) {
                var headers = ['Name', 'Student ID', 'Course', 'Section', 'Supervisor', 'Coordinator', 'Last Logged', 'Status'];
                var lines = [headers.join(',')];
                rows.forEach(function (r) {
                    var vals = [r.name, r.student_id, r.course, r.section, r.supervisor, r.coordinator, r.last_logged, r.status]
                        .map(function (v) {
            ...
            function toTxt(rows) {
                var lines = ['Name | Student ID | Course | Section | Supervisor | Coordinator | Last Logged | Status'];
                rows.forEach(function (r) {
                    lines.push([r.name, r.student_id, r.course, r.section, r.supervisor, r.coordinator, r.last_logged, r.status].join(' | '));
                });
                return lines.join('\n');
            }
            ...
            document.querySelectorAll('.js-export').forEach(function (btn) {
                btn.addEventListener('click', function (e) {
                    ...
                    if (type === 'csv' || type === 'excel') {
                        ...
                        var mime = type === 'excel' ? 'application/vnd.ms-excel;charset=utf-8;' : 'text/csv;charset=utf-8;';
