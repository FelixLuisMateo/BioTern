...
function current_school_year_label(?int $timestamp = null): string {
...
$userCols = table_exists($conn, 'users') ? get_columns($conn, 'users') : [];
        ...
        if ($studentId <= 0) {
            ...
            if ($existingRow) {
                ...
                if (!$studentRow) {
                    ...
                    $studentCols = get_columns($conn, 'students');
                    $internCols = get_columns($conn, 'internships');
                    ...
                    $coordRefId = 0;
                    $supRefId = 0;
                    if (table_exists($conn, 'coordinators')) {
                    ...
                    if (table_exists($conn, 'supervisors')) {
...
(function () {
    ...
    function selectedStudentOption() {
        return studentSelect.options[studentSelect.selectedIndex] || null;
    }
