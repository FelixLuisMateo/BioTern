...
(function () {
  ...
  function applyCurrentYear() {
    var yearNodes = document.querySelectorAll(".biotern-current-year");
    if (!yearNodes.length) {
      return;
    }
  ...
  // Shared bootstrapping for pages that currently duplicate small inline scripts.
  applyStoredSkinClass();

  window.BioTernUnified = window.BioTernUnified || {};
  window.BioTernUnified.applyStoredSkinClass = applyStoredSkinClass;
  window.BioTernUnified.showToast = showToast;
  window.BioTernUnified.applyCurrentYear = applyCurrentYear;
...
/* Footer theme runtime migrated from includes/footer.php */
(function () {
  "use strict";
  ...
  function hydrateThemeRuntimeConfigFromBody() {
    ...
    var prefsRaw = document.body.getAttribute("data-biotern-theme-prefs") || "";
    var apiRaw = document.body.getAttribute("data-biotern-theme-api") || "";
